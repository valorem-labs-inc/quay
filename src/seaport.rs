...
pub mod seaport_mod {
    ...
    use ethers::core::{
        abi::{Abi, AbiDecode, Detokenize, InvalidOutputType, Token, Tokenizable},
        types::*,
    };
    ...
    impl ConsiderationItem {
        pub fn from_db_struct(
            consideration: &DBConsiderationItem,
        ) -> Result<ConsiderationItem, anyhow::Error> {
            Ok(ConsiderationItem {
                item_type: consideration.item_type as u8,
                token: H160::decode_hex(&consideration.token)?,
                identifier_or_criteria: U256::from_str(&consideration.identifier_or_criteria)?,
                start_amount: U256::from_str(&consideration.start_amount)?,
                end_amount: U256::from_str(&consideration.end_amount)?,
                recipient: H160::decode_hex(&consideration.recipient)?,
            })
        }
    }
    ...
    impl OfferItem {
        pub fn from_db_struct(offer: &DBOfferItem) -> Result<OfferItem, anyhow::Error> {
            Ok(OfferItem {
                item_type: offer.item_type as u8,
                token: H160::decode_hex(&offer.token)?,
                identifier_or_criteria: U256::from_str(&offer.identifier_or_criteria)?,
                start_amount: U256::from_str(&offer.start_amount)?,
                end_amount: U256::from_str(&offer.end_amount)?,
            })
        }
    }
    ...
    impl Order {
        pub fn from_db_struct(order: &DBOrder) -> Result<Order, anyhow::Error> {
            Ok(Order {
                parameters: OrderParameters::from_db_struct(order)?,
                signature: Bytes::from_str(&order.signature)?,
            })
        }
    }
    ...
    impl OrderComponents {
        pub async fn from_parameters(
            seaport: &Seaport<Provider<Http>>,
            parameters: &OrderParameters,
        ) -> OrderComponents {
            let counter = seaport
                .get_counter(parameters.offerer)
                .call()
                .await
            ...
            OrderComponents {
                ...
                zone_hash: parameters.zone_hash.0,
                salt: parameters.salt,
                conduit_key: parameters.conduit_key.0,
                counter,
            }
        }
    }
    ...
    pub struct OrderParameters {
        ...
        pub start_time: ethers::core::types::U256,
        pub end_time: ethers::core::types::U256,
        pub zone_hash: ethers::core::types::H256,
        pub salt: ethers::core::types::U256,
        pub conduit_key: ethers::core::types::H256,
        pub total_original_consideration_items: ethers::core::types::U256,
    }
    impl OrderParameters {
        pub fn from_db_struct(order: &DBOrder) -> Result<OrderParameters, anyhow::Error> {
            Ok(OrderParameters {
                offerer: H160::decode_hex(&order.offerer)?,
                zone: H160::decode_hex(&order.zone)?,
                order_type: order.order_type as u8,
                start_time: U256::from(order.start_time),
                end_time: U256::from(order.end_time),
                zone_hash: H256::decode_hex(&order.zone_hash)?,
                salt: U256::from_dec_str(&order.salt)?,
                conduit_key: H256::decode_hex(&order.conduit_key)?,
                total_original_consideration_items: U256::from(
                    order.total_original_consideration_items,
                ),

                offer: order
                    .offers
                    .iter()
                    .map(OfferItem::from_db_struct)
                    .collect::<Result<Vec<OfferItem>, anyhow::Error>>()?,
                consideration: order
                    .considerations
                    .iter()
                    .map(ConsiderationItem::from_db_struct)
                    .collect::<Result<Vec<ConsiderationItem>, anyhow::Error>>()?,
            })
        }
    }
